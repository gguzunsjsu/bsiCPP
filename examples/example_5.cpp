...
int main() {
    ...
    for (int i = 0; i < vector_length; ++i) {
        ...
        one_dec[i] = ((zipf_dist(gen) - 2)) / 100.0; // Range: up to ~5.0
        two_dec[i] = ((zipf_dist(gen) - 2)) / 100.0;

        one_dec_signed[i] = (zipf_dist(gen) - (range / 2)) / 100.0;
        two_dec_signed[i] = (zipf_dist(gen) - (range / 2)) / 100.0;
    }
    ...
    for (int i = 0; i < vector_length; ++i) {
        ...
        normal_sum_d[i] = one_dec[i] + two_dec[i];
        mixed_sum_d_signed_plus_unsigned[i] = one_dec_signed[i] + two_dec[i];
    ...
    BsiVector<uint64_t>* bsi_one_dec = bsi.buildBsiVector(one_dec, precision, 0.2);
    bsi_one_dec->setFirstSliceFlag(true);
    bsi_one_dec->setLastSliceFlag(true);
    bsi_one_dec->setPartitionID(0);

    BsiVector<uint64_t>* bsi_two_dec = bsi.buildBsiVector(two_dec, precision, 0.2);
    bsi_two_dec->setFirstSliceFlag(true);
    bsi_two_dec->setLastSliceFlag(true);
    bsi_two_dec->setPartitionID(0);

    BsiVector<uint64_t>* bsi_one_dec_signed = s_bsi.buildBsiVector(one_dec_signed, precision, 0.2);
    bsi_one_dec_signed->setFirstSliceFlag(true);
    bsi_one_dec_signed->setLastSliceFlag(true);
    bsi_one_dec_signed->setPartitionID(0);

    BsiVector<uint64_t>* bsi_two_dec_signed = s_bsi.buildBsiVector(two_dec_signed, precision, 0.2);
    bsi_two_dec_signed->setFirstSliceFlag(true);
    bsi_two_dec_signed->setLastSliceFlag(true);
    bsi_two_dec_signed->setPartitionID(0);
    ...
    // int x = bsi_two->numSlices;
    // A = bsi_one->bsi[0].And(bsi_two->bsi[x-1]);
    //
    // B = bsi_one->bsi[0].Or(bsi_two->bsi[x-1]);
    //
    // C = bsi_one->bsi[0].Xor(bsi_two->bsi[x-1]);
    //
    // // D = bsi_one->bsi[0].andNot(bsi_two->bsi[x-1]);
    // D = bsi_two->bsi[x-1].andNot(bsi_one->bsi[0]);
