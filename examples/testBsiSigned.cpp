//
// Created by Cindy Zhang on 6/3/23.
//

#include "testBsiSigned.h"
#include "BsiAttribute.hpp"
#include "BsiSigned.hpp"
#include "BsiUnsigned.hpp"

using namespace std;

int main() {
    BsiSigned<uint64_t> build;
    vector<long> array{-1,1};

    build.buildBsiAttributeFromVectorSigned(array,0.2);
    //bsi->TwosToSignMagnitude();
    for (int i=0; i<array.size(); i++) {
        cout << build.sign.get(i) << build.getValue(i) << " ";
    }
    cout << endl;
    cout << build.existenceBitmap.sizeInBits() << "\n";
    HybridBitmap<uint64_t> topkmax = build.topKMax(2);
    cout << topkmax.numberOfOnes() << "\n";
    cout << topkmax.sizeInBits();
    return 0;
}
