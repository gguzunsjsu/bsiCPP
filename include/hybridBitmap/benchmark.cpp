...
public:
  ...
  vector<uint32_t> generateClustered(uint32_t N, uint32_t Max) {
    if (N > Max)
      throw runtime_error("can't generate so many values");
...
template <class uword> void test(vector<vector<uint32_t>> &data, int repeat) {
  WallClockTimer timer;
  long bogus = 0;
  // building
  timer.reset();
  vector<HybridBitmap<uword>> ewah(data.size());
  cout << "# displaying size, creation time, convert to array time, OR time, "
          "AND time, and a bogus value "
       << endl;
  ...
  for (int r = 0; r < repeat; ++r)
    for (size_t k = 0; k < data.size(); ++k) {
      HybridBitmap<uword> ewahor(ewah[0]);
  ...
  for (int r = 0; r < repeat; ++r)
    for (size_t k = 0; k < data.size(); ++k) {
      HybridBitmap<uword> ewahand(ewah[0]);
      for (size_t j = 1; j < k; ++j) {
        HybridBitmap<uword> container;
