...
public:
    ...
    */
    ...
    virtual HybridBitmap<uword> reLU(const BsiVector<uword> *a)const=0;
     ...
     */
    virtual std::pair<BsiVector<uword>*, BsiVector<uword>*> divide(
            const BsiVector<uword>& dividend,
            const BsiVector<uword>& divisor) const = 0;
    std::pair<BsiVector<uword>*, BsiVector<uword>*> divide(
            BsiVector<uword>* divisor) const;
    ...
    BsiVector<uword>* createRandomBsi(int vectorLength, int range, double compressThreshold) const;
    ...
    BsiVector<uword>* maskAssign(BsiVector<uword>* other,const HybridBitmap<uword> &mask);
    BsiVector<uword>* shift(int k) const;
...
//        total += existenceBitmap.getSizeInMemory();
    ...
    */
    ...
    BsiVector<uword>* operator+(BsiVector<uword>* other);
    BsiVector<uword>* operator*(const BsiVector<uword>* other);
    BsiVector<uword>* operator*(int other)const;
//    BsiVector<uword>* operator/(const BsiVector<uword>* other);
...
template <class uword>
BsiVector<uword>* BsiVector<uword>::createRandomBsi(int vectorLength, int range, double compressThreshold) const {
...
BsiVector<uword>* BsiVector<uword>::buildBsiVector(std::vector<long> nums, double compressThreshold) const{
    ...
    if (min < 0) {
        ...
        //The method to put the elements in the input vector nums to the bsi property of BsiVector result
        std::vector< std::vector< uword > > bitSlices = bringTheBits(nums,slices+2,numberOfElements);
        size_t compressedSlices = 0;
        size_t verbatimSlices = 0;
...
template <class uword>
void BsiVector<uword>::applyExistenceBitmap(const HybridBitmap<uword> &ex){
    existenceBitmap = ex;
    for(int i=0; i < numSlices; i++){
 ...
 */
template <class uword>
BsiVector<uword>* BsiVector<uword>::maskAssign(BsiVector<uword>* other,const HybridBitmap<uword> &mask) {
...
template <class uword>
BsiVector<uword>* BsiVector<uword>::operator+(BsiVector<uword>* other) {
    return this->SUM(other);
}

template <class uword>
BsiVector<uword>* BsiVector<uword>::operator*(const BsiVector<uword>* other) {
    return this->multiplyWithBsiHorizontal(other);
}

template <class uword>
BsiVector<uword>* BsiVector<uword>::operator*(int other) const {
 ...
 */
template <class uword>
BsiVector<uword>* BsiVector<uword>::shift(int k) const {
    int slices = bsi.size();
    BsiSigned<uword>* res = new BsiSigned<uword>(slices);
...
template <class uword>
std::pair<BsiVector<uword>*, BsiVector<uword>*> BsiVector<uword>::divide(
        BsiVector<uword>* divisor) const {
