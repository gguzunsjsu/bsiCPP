...
template <class uword> bool testInEqualityEWAHBoolArray() {
  ...
  HybridBitmap<uword> b4 = HybridBitmap<uword>::bitmapOf(4, 1, 10, 11, 10000);
...
template <class uword> bool testEmpty() {
  cout << "[testing SizeInBits] sizeof(uword)=" << sizeof(uword) << endl;
    HybridBitmap<uword> ewah0;
    if(!ewah0.empty()) {
      std::cout << "empty is buggy " << std::endl;
      return false;
    }
    HybridBitmap<uword> ewah1 = HybridBitmap<uword>::bitmapOf(1, 1);
    if(ewah1.empty()) {
      std::cout << "empty is buggy " << std::endl;
      return false;
    }
...
template <class uword> bool testSizeInBits() {
  cout << "[testing SizeInBits] sizeof(uword)=" << sizeof(uword) << endl;
    ...
    HybridBitmap<uword> ewah1 = HybridBitmap<uword>::bitmapOf(1, 1);
    HybridBitmap<uword> ewah2 = HybridBitmap<uword>::bitmapOf(1, 2);
    HybridBitmap<uword> ewah3 = HybridBitmap<uword>::bitmapOf(1, 3);
    HybridBitmap<uword> ewah4 = HybridBitmap<uword>::bitmapOf(1, 4);
    HybridBitmap<uword> result1 = ewah1 | ewah2 | ewah3 | ewah4;
    HybridBitmap<uword> result12 = ewah1 & ewah2 & ewah3 & ewah4;
    HybridBitmap<uword> result13 = ewah1 ^ ewah2 ^ ewah3 ^ ewah4;

    HybridBitmap<uword> result2 = HybridBitmap<uword>::bitmapOf(4, 1, 2, 3, 4);
    if(result1 != result2) {
...
template <class uword> bool testSerialSize() {
  ...
  size_t bitval = 55555;
  HybridBitmap<uword> b =
      HybridBitmap<uword>::bitmapOf(1, bitval); // set just one bit
  // generic write method in HybridBitmap can be wasteful for single bits...
  stringstream ss;
  b.write(ss);
...
template <class uword> bool testIntersects() {
  ...
  HybridBitmap<uword> b1 = HybridBitmap<uword>::bitmapOf(2, 2, 1000);
  cout << "constructing b2..." << endl;

  HybridBitmap<uword> b2 = HybridBitmap<uword>::bitmapOf(2, 1000, 1100);
  cout << "constructing b3..." << endl;

  HybridBitmap<uword> b3 = HybridBitmap<uword>::bitmapOf(2, 1100, 2000);
...
template <class uword> bool testCardinalityEWAHBoolArray() {
  cout << "[testing CardinalityEWAHBoolArray] sizeof(uword)=" << sizeof(uword)
       << endl;
  HybridBitmap<uword> b1 = HybridBitmap<uword>::bitmapOf(1, 1);
  if (b1.numberOfOnes() != 1) {
    return false;
  }
  b1.inplace_logicalnot();
  ...
  HybridBitmap<uword> b = HybridBitmap<uword>::bitmapOf(2, 1, 100);
  if (b.numberOfOnes() != 2) {
    cout << "b " << b << endl;
    return false;
  }
  HybridBitmap<uword> bout;
  b.logicalnot(bout);
  if (bout.numberOfOnes() != 99) {
    cout << "bout " << bout << endl;
    return false;
...
template <class uword> bool testLargeDirty() {
  ...
  HybridBitmap<uword> b1 = HybridBitmap<uword>();
  HybridBitmap<uword> b2 = HybridBitmap<uword>();
  ...
  HybridBitmap<uword> b3 = HybridBitmap<uword>::bitmapOf(3, 1, 10, 1001);
  HybridBitmap<uword> b4 = HybridBitmap<uword>::bitmapOf(3, 1, 10, 1001);
...
template <class uword> bool testSetGet() {
  ...
  size_t counter = 0;
  bool isOk = true;
  for (typename HybridBitmap<uword>::const_iterator i = ewcb.begin();
       i != ewcb.end(); ++i) {
    if (val[counter++] != *i) {
...
template <class uword> bool testEWAHBoolArrayAppend() {
  ...
  HybridBitmap<uword> caggregate1;
  caggregate1.append(myarray1);
  HybridBitmap<uword> caggregate2;
...
template <class uword> bool testJoergBukowski() {
  ...
  HybridBitmap<uword> mytestarray;
  for (vector<uint32_t>::const_iterator i = positions.begin();
       i != positions.end(); ++i) {
    mytestarray.set(*i);
  }
  size_t count = 0;
  for (typename HybridBitmap<uword>::const_iterator j = mytestarray.begin();
       j != mytestarray.end(); ++j) {
  ...
  for (vector<uint32_t>::const_iterator i = positions.begin();
    ...
    vals.clear();
    for (typename HybridBitmap<uword>::const_iterator j = recovered.begin();
         j != recovered.end(); ++j) {
...
bool testPhongTran() {
  ...
  myarray.write(out);
  out.close();
  HybridBitmap<uint32_t> lmyarray;
  ifstream in(indexfile.c_str(), ios::binary);
  lmyarray.read(in);
  in.close();
  HybridBitmapIterator<uint32_t> i = myarray.uncompress();
  HybridBitmapIterator<uint32_t> j = lmyarray.uncompress();
...
template <class uword> bool testHemeury() {
  ...
  bool isOk(true);
  HybridBitmap<uword> test, test1, test2;
...
template <class uword> bool testPhongTran2() {
  ...
  HybridBitmapIterator<uword> i = myarray.uncompress();
  HybridBitmapIterator<uword> j = lmyarray.uncompress();
  while (i.hasNext()) {
    if (!j.hasNext()) {
      cout << "the two arrays don't have the same size?" << endl;
      isOk = false;
...
template <class uword> bool testEWAHBoolArray() {
  cout << "[testing HybridBitmap]" << endl;
  bool isOk(true);
  HybridBitmap<uword> myarray;
...
template <class uword> bool testSTLCompatibility() {
  ...
  bitset1.set(1000);
  bitset1.set(1001);
  vector<HybridBitmap<uword> > testVec(1);
  testVec[0].set(1);
  testVec[0].set(2);
  testVec[0].set(1000);
  testVec[0].set(1001);
...
template <class uword> bool testEWAHBoolArrayLogical() {
  ...
  HybridBitmap<uword> myand;
  HybridBitmap<uword> myor;
  HybridBitmap<uword> myxor;
  HybridBitmap<uword> myxoralt;

  myarray1.logicaland(myarray2, myand);
  myarray1.logicalor(myarray2, myor);
  myarray1.logicalxor(myarray2, myxor);
  HybridBitmap<uword> tmp(myand);
  ...
  HybridBitmapIterator<uword> i = myand.uncompress();
  HybridBitmapIterator<uword> j = myor.uncompress();
  HybridBitmapIterator<uword> it1 = myarray1.uncompress();
  HybridBitmapIterator<uword> it2 = myarray2.uncompress();
  for (uint32_t k = 0; k < usedN; ++k) {
...
void init(HybridBitmap<uword> &ba, size_t N, size_t x[]) {
  for (size_t ix = 0; ix < N; ++ix)
    ba.set(x[ix]);
}

template <class uword>
std::ostream &operator<<(std::ostream &os, const HybridBitmap<uword> &ba) {
  os << " (" << ba.sizeInBits() << ") ";
  typename HybridBitmap<uword>::const_iterator it = ba.begin(),
                                                last = ba.end();
  ...
  size_t ixBit = 0;
  const uword wordInBits = HybridBitmap<uword>::wordinbits;

  HybridBitmapRawIterator<uword> ir = ba.raw_iterator();
...
template <class uword> bool testSerialization() {
  ...
  for (int k = 0; k < 10; ++k) {
    ...
    typename HybridBitmap<uword>::const_iterator i = bitmap.begin();
    typename HybridBitmap<uword>::const_iterator j = lmyarray.begin();
...
template <class uword> bool testEWAHBoolArrayLogical2() {
  ...
  HybridBitmap<uword> ba1, ba2, baAND, baOR, baXOR, testAND, testOR, testXOR;
...
template <class uword> bool testFastOrAggregate() {
  ...
  HybridBitmap<uword> ba1, ba2, ba3, baOR;
  ...
  HybridBitmap<uword> longway = ba1.logicalor(ba2).logicalor(ba3);
  const HybridBitmap<uword> *mybitmaps[] = {&ba3, &ba2, &ba1};
  HybridBitmap<uword> fastway =
...
public:
  ...
  static HybridBitmap<uint64_t> **readFile(string filename) {
    HybridBitmap<uint64_t> **ewahs = new HybridBitmap<uint64_t> *[2];
  ...
  static HybridBitmap<uint64_t> *readOneBitmap(ifstream *inputStream) {
    HybridBitmap<uint64_t> *ewah = new HybridBitmap<uint64_t>;
...
bool testRealData() {
  ...
  HybridBitmap<uint64_t> container;
  ...
  for (size_t k = 0; k < 207; ++k) {
    ...
    HybridBitmap<uint64_t> **ewahs = JavaEWAHReader::readFile(filename);
    ...
    HybridBitmap<uint64_t> tmpcontainer;

    ewahs[0]->logicalxor(*ewahs[1], tmpcontainer);
    HybridBitmap<uint64_t> createdandnotcontainer;
    ewahs[0]->logicaland(tmpcontainer, createdandnotcontainer);
    HybridBitmap<uint64_t> directandnotcontainer;
...
template <class uword> bool dataindexingtest() {
  ...
  map<string, HybridBitmap<uword> > index1;
  map<string, HybridBitmap<uword> > index2;
  ...
  for (typename map<string, HybridBitmap<uword> >::iterator i = index1.begin();
       i != index1.end(); ++i) {
    HybridBitmap<uword> &bitmap1 = i->second;
    for (typename map<string, HybridBitmap<uword> >::iterator j =
             index2.begin();
         j != index2.end(); ++j) {
      HybridBitmap<uword> &bitmap2 = j->second;
      bool intersect = bitmap1.intersects(bitmap2);
      HybridBitmap<uword> result;
      testcount++;
...
typedef struct { HybridBitmap<uint32_t> gpr[32][4]; } vcpu_ctx;
...
template <class uword> bool arrayinit2d() {
  cout << "[arrayinit2d] checking arrayinit...sizeof(uword)=" << sizeof(uword)
       << endl;
  HybridBitmap<uint32_t> gpr[10][4];
  for (int k = 0; k < 10; ++k)
    for (int l = 0; l < 4; ++l)
      gpr[k][l].set(k);
  for (int k = 0; k < 10; ++k)
