...
public:
  ...
  //frbt(const RunningLengthWord &rlw) : mydata(rlw.mydata) {}
...
public:
  ...
  BufferedRunningLengthWord(const uword &data,
                            HybridBitmapRawIterator<uword> *p)
  ...
  void discharge(HybridBitmap<uword> &container) {
    while (size() > 0) {
      // first run
  ...
  size_t discharge(HybridBitmap<uword> &container, size_t max) {
    size_t index = 0;
    while (true) {
      if (index + RunningLength > max) {
        const size_t offset = max - index;
...
size_t dischargeNegatedDecompressed(HybridBitmap<uword> &container, size_t max) {
    size_t index = 0;
    while ((index < max) && (size() > 0)) {
        // first run
        size_t pl = getRunningLength();
...
//            container.buffer[container.actualsizeinwords+i]=~this.buffer[this.literalWordStartPosition+i];
  ...
  // Write out up to max words, returns how many were written
  size_t dischargeNegated(HybridBitmap<uword> &container, size_t max) {
  ...
  void writeLiteralWords(size_t numWords, HybridBitmap<uword> &container) {
...
private:
  ...
  HybridBitmapRawIterator<uword> *parent;
};
